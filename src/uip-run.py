...
def main(args):
    ...
    conn.connect()
    cmdline=" ".join(args[2:])
    print (inverse_seq + "running: " + normal_seq + remote_path + " " +inverse_seq + "\nTOS argv:" + normal_seq +" "+ cmdline)
    conn.set_debuglevel(1)
    conn.request("GET", remote_path + "?run=" + cmdline)
    ...
    if response.status != 200:
        ...
        print "HTTP OK (" + str(response.status) + ")"
