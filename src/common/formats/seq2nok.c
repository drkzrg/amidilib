...
// create file header
   ...
   amMemCpy(out, &nktHead, sizeof(sNktHd));
   out += sizeof(sNktHd);
...
// process tracks
    ...
    if(pOutFileName){
       ...
       // fwrite(midiTrackHeaderOut - sizeof(sNktHd), bytes_written, 1, file);
       fclose(file);
